...
with open('weatherHistory.csv') as csv_file:
	...
	for row in csv_reader:
		if not row[0]=='Formatted Date':
			...
			if row[1]=='Overcast' and len(X1)<1000  and temp>0 and float(row[5])*100<=170*(25-float(row[3]))/25:
				inputList.append([temp,RH,var])
				outputList.append(1)

				X1.append(temp)
				Y1.append(RH)

			elif row[1]=='Clear' and len(X2)<1000 and float(row[5])*100>130*(25-float(row[3]))/25:
				inputList.append([temp,RH,var])
				outputList.append(0)
				X2.append(temp)
				Y2.append(RH)
...
Z = float(temp) * W[1] + float(RH) * W[0] + 35 * W[2] + b
a = 1/(1+exp(-Z))
print(a)
