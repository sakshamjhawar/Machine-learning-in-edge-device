...
with open('Rain.csv') as csv_file:
	...
	for row in csv_reader:
		...
		pressure = float(row[10])/10
		if row[1]=='Mostly Cloudy' or row[1]=='Overcast':
			X1.append(temp)
			Y1.append(RH)
			inputList.append([temp,RH,pressure])
			outputList.append(1)
		else:
			RH = RH/2
			X2.append(temp)
...
print()
print("Variables W and b are ",W[0],W[1],W[2],b)#----I want to delete this too!
...
pressure = input('Pressure in millibars :')
...
Z = float(temp) * W[0] + float(RH) * W[1] + float(pressure) /10 * W[2] + b
a = 1/(1+exp(-Z))
print(a)
